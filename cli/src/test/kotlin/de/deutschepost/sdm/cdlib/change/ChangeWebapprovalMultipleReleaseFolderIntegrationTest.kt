...
class ChangeWebapprovalMultipleReleaseFolderIntegrationTest(
    ...
    private val envVariables = mutableMapOf(
        //"CDLIB_BUILD_URL" to "test build url: https://integrationtest.jenkinsbuildurl.de",
    ...
    init {
        context("Creating webapproval change with multiple release folders is a success") {
            ...
            val args: Array<String> =
                ("change create --jira-token $token --debug " +
                    ...
                    "--folder-name $fortifyName --folder-name $owaspName --folder-name $zapName --folder-name $zapName2 --sharepoint-username $sp_username --sharepoint-password $sp_password " +
            ...
            test("Reports exist in artifactory") {
                ...
                listOf(fortifyName, owaspName, zapName, zapName2).forEach {
                    withClue(it) {
    ...
    override suspend fun beforeSpec(spec: Spec) {
        ...
        withEnvironment(envVariables, OverrideMode.SetOrOverride) {
            ...
            val reportFolder2 = uploadReportsToArtifactory(
                artifactoryClient, repoName,
                zapName = zapName2,
            )
            zapName2 = reportFolder2.zapName
        }
