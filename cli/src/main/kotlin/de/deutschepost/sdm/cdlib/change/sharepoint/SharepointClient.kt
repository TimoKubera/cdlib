...
open class SharepointClient(private val user: String, private val password: String) {
    ...
    private fun getDigest(): String? {
        val httpPost = HttpPost("$ISHARE_WEBAPPROVAL_BASE_URL/_api/contextinfo").apply {
            addHeader("Accept", JSON_HEADER_VALUE)
        }
    ...
    private fun addEntry(
        ...
        val digest =
            checkNotNull(getDigest()) {
                ...
                curl -v --ntlm -u 'USER:PASSWORD' \"$ISHARE_WEBAPPROVAL_BASE_URL/_api/web/lists/GetByTitle('Pipeline%20Approvals')\" -H "Accept: ${JSON_HEADER_VALUE}"
                """.trimIndent()
            }
        ...
        val httpPost = HttpPost("$ISHARE_WEBAPPROVAL_BASE_URL/_api/web/lists/GetByTitle('$listName')/items").apply {
            ...
            addHeader("Content-Type", JSON_HEADER_VALUE)
            entity = httpEntity
        }
    ...
    companion object : KLogging() {
        private const val JSON_HEADER_VALUE = "application/json;odata=verbose"
