    ...
    class FetchCommand : SubcommandWithHelp() {
        ...
        class CSSCommand : SubcommandWithHelp(), Callable<Int> {
            ...
            override fun call(): Int = runBlocking {
                ...
                val (registry, project, repositoryReference) = splitImageString(image)
                    ?: return@runBlocking -1
            
                validateRegistry(registry)
                    ?: return@runBlocking -1
            
                val (repository, reference) = parseRepositoryReference(repositoryReference)
                    ?: return@runBlocking -1
                ...
                checkImagePresenceInHarbor(registry, project, repository, reference, auth)
                    ?: return@runBlocking -1
            
                waitForScanCompletion(registry, project, repository, reference, auth, cssTimeout)
                    ?: return@runBlocking -1
                ...
                val ccaSuppressionList = getCveAllowList(registry, project, auth)
                    ?: return@runBlocking -1
            ...
            private fun splitImageString(image: String): Triple<String, String, String>? {
                return runCatching {
            ...
            private fun checkImagePresenceInHarbor(
                registry: String,
                project: String,
                repository: String,
                reference: String,
            ...
            private fun getCveAllowList(registry: String, project: String, auth: BasicAuth): Any? {
                return runCatching {
        ...
        class FnciCommand : SubcommandWithHelp(), Callable<Int> {
            ...
            override fun call(): Int = runBlocking(Dispatchers.IO) {
                runCatching {
                    withTimeoutOrNull(fnciTimeout.toDuration(DurationUnit.MINUTES)) {
                        ...
                        val (projectInfo, inventory) = startProjectJobs()
                        ...
                        handleReportJobResult(fetchReportLoop(taskId)).let {
                            when (it) {
            ...
            private suspend fun startProjectJobs(): Pair<Any, Any> {
                ...
                return Pair(projectInfo, inventory)
            }
            
            private suspend fun fetchReportLoop(taskId: String): ByteArray? = async {
            ...
            private fun handleReportJobResult(result: ByteArray?) {
                result?.let {
