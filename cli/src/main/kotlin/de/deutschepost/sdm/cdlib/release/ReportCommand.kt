    ...
    class FetchCommand : SubcommandWithHelp() {
        ...
        class CSSCommand : SubcommandWithHelp(), Callable<Int> {
            ...
            override fun call(): Int = runBlocking {
                ...
                fun parseImageComponents(image: String): Triple<String, String, String> {
                    return image.split("/", limit = 3).apply { require(size == 3) }.let { Triple(it[0], it[1], it[2]) }
                }
            
                fun parseRepositoryReference(repositoryReference: String): Pair<String, String> {
                    return repositoryReference.replace("/", "%2F").split(":", limit = 2).apply { require(size == 2) }.let { Pair(it[0], it[1]) }
                }
            
                suspend fun pollScanStatus(): Int? {
                ...
                when (val pollResult = pollScanStatus()) {
                    0 -> logger.info { "Harbor CCA scan completed" }
                    null -> {
