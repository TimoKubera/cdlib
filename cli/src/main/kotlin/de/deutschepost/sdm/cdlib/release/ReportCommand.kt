    ...
    class FetchCommand : SubcommandWithHelp() {
        ...
        class CSSCommand : SubcommandWithHelp(), Callable<Int> {
            ...
            override fun call(): Int = runBlocking {
                ...
                if (!isValidRegistry(registry)) return@runBlocking -1
                ...
                if (!isImagePresentInHarbor(registry, project, repository, reference, auth)) return@runBlocking -1
            ...
            private fun isValidRegistry(registry: String): Boolean {
            ...
            private fun isImagePresentInHarbor(registry: String, project: String, repository: String, reference: String, auth: BasicAuth): Boolean {
            ...
            private fun waitForScanCompletion(registry: String, project: String, repository: String, reference: String, auth: BasicAuth): Boolean {
            ...
            private fun fetchCcaReport(registry: String, project: String, repository: String, reference: String, auth: BasicAuth): Any? {
                return runCatching {
        ...
        class FnciCommand : SubcommandWithHelp(), Callable<Int> {
            ...
            override fun call(): Int = runBlocking(Dispatchers.IO) {
                runCatching {
                    withTimeoutOrNull(fnciTimeout.toDuration(DurationUnit.MINUTES)) {
                        ...
                        val reportJob = createReportJob(taskId)
                        ...
                        handleReportJob(reportJob, releaseName)
                    }.let {
                        when (it) {
                            null -> {
            ...
            private fun CoroutineScope.createReportJob(taskId: String) = async {
            ...
            private suspend fun handleReportJob(reportJob: Deferred<Any?>, releaseName: String) {
                reportJob.await().let {
                    when (it) {
                        is ByteArray -> {
