    ...
    class CreateCommand : SubcommandWithHelp(), Callable<Int> {
        ...
        * So the ArgGroup needs and order with an even number, e.g. 6, and any group that should be placed under this group gets the order one higher, e.g. 7. That way all options appear together under the same heading.
        ...
        override fun call(): Int {
            ...
            runCatching {
                initiateChangeManagement(reportUrl)
            }.onFailure {
                it.klogSelf(logger)
                return -1
            }.getOrThrow()
        
            val webapprovalUrl = handleWebapprovalUrl(reportUrl)
        ...
        private fun verifyReports(originalArgs: List<String>): String {
            ...
            handleWebapproval(nameToAllReports)
        
            if (oslcSection.performOslc) {
                performOSLCComplianceCheck(nameToBuildReports)
            }
        
            logger.info { "Generating and copying immutable artifacts..." }
            return artifactoryMixinFull.copyFiles().also { reportFolderUrl ->
        ...
        private fun handleWebapproval(nameToAllReports: Map<String, List<Report>>) {
        ...
        private fun performOSLCComplianceCheck(nameToBuildReports: Map<String, List<Report>>) {
        ...
        private fun initiateChangeManagement(reportUrl: String?) {
        ...
        private fun handleWebapprovalUrl(reportUrl: String?): String? {
        ...
        private fun handleOSLCUrls(): List<String> {
