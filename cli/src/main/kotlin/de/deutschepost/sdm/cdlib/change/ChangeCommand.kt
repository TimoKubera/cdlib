        ...
        * So the ArgGroup needs and order with an even number, e.g. 6, and any group that should be placed under this group gets the order one higher, e.g. 7. That way all options appear together under the same heading.
        ...
        override fun call(): Int {
            ...
            val reportUrl = if (webApprovalSection.performWebapproval or oslcSection.performOslc) {
                ...
                val url = performReleaseVerification(originalArgs)
        ...
        private fun checkVersionInfo(versionInfo: Any) {
            if (!(versionInfo as dynamic).isSupported) {
            ...
            if (!(versionInfo as dynamic).isLatest) {
        ...
        private fun performChangeManagement(): Boolean {
            return runCatching {
