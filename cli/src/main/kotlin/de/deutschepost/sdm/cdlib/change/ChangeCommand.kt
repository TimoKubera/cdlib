    ...
    class CreateCommand : SubcommandWithHelp(), Callable<Int> {
        ...
        * So the ArgGroup needs and order with an even number, e.g. 6, and any group that should be placed under this group gets the order one higher, e.g. 7. That way all options appear together under the same heading.
        ...
        override fun call(): Int {
            fun performReleaseVerification(originalArgs: List<String>): String {
            ...
            fun processChangeManagement() {
                changeHandler.initialise(
                    authToken = auth,
                    isTestFlag = isTest,
            ...
            fun processWebApproval(reportUrl: String?): String {
            ...
            val reportUrl: String? = if (webApprovalSection.performWebapproval || oslcSection.performOslc) {
            ...
            val webapprovalUrl: String? = if (webApprovalSection.performWebapproval) {
                logger.info { "Flag --webapproval found. Adding Sharepoint entry." }
                try {
                    processWebApproval(reportUrl)
                } catch (e: Exception) {
