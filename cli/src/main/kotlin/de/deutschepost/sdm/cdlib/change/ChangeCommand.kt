    ...
    class CreateCommand : SubcommandWithHelp(), Callable<Int> {
        ...
        * So the ArgGroup needs and order with an even number, e.g. 6, and any group that should be placed under this group gets the order one higher, e.g. 7. That way all options appear together under the same heading.
        ...
        override fun call(): Int {
            ...
            val reportUrl = handleReleaseVerification(originalArgs)
        
            logger.debug { "CDLib version: $versionInfo" }
            handleChangeManagementProcess()
        
            val webapprovalUrl = handleWebApproval(reportUrl)
        
            handleTqsFlag()
        
            val oslcURls = handleOslcEntry()
        
            handleGeneratedUrlsFile(reportUrl, webapprovalUrl, oslcURls)
        
            return 0
        }
        
        private fun handleReleaseVerification(originalArgs: List<String>): String? {
            return if (webApprovalSection.performWebapproval || oslcSection.performOslc) {
        ...
        private fun verifyReports() {
            ...
            if (webApprovalSection.performWebapproval) {
                ...
                verifyWebApprovalReports(nameToAllReports)
            }
        }
        
        private fun verifyWebApprovalReports(nameToAllReports: Map<String, List<Report>>) {
            ...
            concludeWebApprovalChecks(hasInvalidReports, hasMissingBuildReports, hasDastReport, hasScaReport, hasSastReport)
        }
        
        private fun concludeWebApprovalChecks(
        ...
        private fun generateAndCopyArtifacts(): String {
            logger.info { "Generating and copying immutable artifacts..." }
            val reportFolderUrl = artifactoryMixinFull.copyFiles()
            val reports = artifactoryMixinFull.artifactoryReports.flatMap { it.value }
        ...
        private fun handleChangeManagementProcess() {
            runCatching {
        ...
        private fun handleWebApproval(reportUrl: String?): String? {
        ...
        private fun handleOslcEntry(): List<String> {
        ...
        private fun handleGeneratedUrlsFile(reportUrl: String?, webapprovalUrl: String?, oslcURls: List<String>) {
