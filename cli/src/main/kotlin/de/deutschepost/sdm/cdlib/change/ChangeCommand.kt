    ...
    class CreateCommand : SubcommandWithHelp(), Callable<Int> {
        ...
        * So the ArgGroup needs and order with an even number, e.g. 6, and any group that should be placed under this group gets the order one higher, e.g. 7. That way all options appear together under the same heading.
        ...
        override fun call(): Int {
            ...
            initiateChangeManagement(auth, reportUrl)
        
            val webapprovalUrl = handleWebapproval(reportUrl)
        
            handleTqsSection()
        
            val oslcURls = handleOslcSection(isTest)
        
            writeOutputUrlsToFile(reportUrl, webapprovalUrl, oslcURls)
        
            return 0
        }
        
        private fun performReleaseVerification(originalArgs: Array<String>): String? {
            ...
            return runCatching {
                ...
                checkAndPrepareReports().let { (nameToAllReports, reportFolderUrl) ->
        ...
        private fun checkAndPrepareReports(): Pair<Map<String, List<Report>>, String> {
            ...
            if (webApprovalSection.performWebapproval) {
                handleWebApprovalSection(nameToAllReports)
            }
        
            verifyOslcReports(nameToBuildReports)
        
            return Pair(nameToAllReports, artifactoryMixinFull.copyFiles())
        }
        
        private fun handleWebApprovalSection(nameToAllReports: Map<String, List<Report>>) {
        ...
        private fun verifyOslcReports(nameToBuildReports: Map<String, List<Report>>) {
            if (!oslcSection.performOslc) return
        ...
        private fun initiateChangeManagement(auth: String, reportUrl: String?) {
            runCatching {
        ...
        private fun handleOslcSection(isTest: Boolean): List<String> {
        ...
        private fun writeOutputUrlsToFile(reportUrl: String?, webapprovalUrl: String?, oslcURls: List<String>) {
            changeManagementSection.outputUrlsFile?.let { outputUrlFile ->
                val file = File(outputUrlFile)
                if (file.exists()) {
                    logger.warn { "File $outputUrlFile already exists and will be overridden." }
                }
