    ...
    class CreateCommand : SubcommandWithHelp(), Callable<Int> {
        ...
        * So the ArgGroup needs and order with an even number, e.g. 6, and any group that should be placed under this group gets the order one higher, e.g. 7. That way all options appear together under the same heading.
        ...
        override fun call(): Int {
            ...
            val reportUrl = if (webApprovalSection.performWebapproval or oslcSection.performOslc) {
                logger.info { "Starting release verification..." }
                if (isTest) {
                    logger.info { "Test run!" }
                }
                try {
                    performReleaseVerification(originalArgs)
                } catch (e: Exception) {
                    return -1
                }
            } else {
            ...
            try {
                performChangeManagementProcess()
            } catch (e: Exception) {
                return -1
            }
            ...
            val webapprovalUrl = if (webApprovalSection.performWebapproval) {
                ...
                try {
                    performWebapprovalEntry(reportUrl)
                } catch (e: Exception) {
                    return -1
                }
            } else {
        ...
        private fun performReleaseVerification(originalArgs: List<String>): String? {
            return runCatching {
                ...
                if (oslcSection.performOslc) {
                    val oslcReportAppNames = artifactoryMixinFull.oslcReports.map { it.name }
                ...
                artifactoryMixinFull.uploadSuppressions(reports.map { it.test }.securityTestsSuppressions())
                ...
                it.klogSelf(logger)
                throw Exception("Release verification error")
            }
        }
        
        private fun performChangeManagementProcess() {
            runCatching {
                ...
                it.klogSelf(logger)
                throw Exception("Change management error")
            }.getOrThrow()
        }
        
        private fun performWebapprovalEntry(reportUrl: String?): String? {
            return runCatching {
                ...
                it.klogSelf(logger)
                throw Exception("Webapproval error")
            }
        }
        
        private fun performOslcEntry(): List<String> {
            return runCatching {
                ...
                it.klogSelf(logger)
                throw Exception("OSLC entry error")
            }
        }
