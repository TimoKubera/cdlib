...
class ChangeCommand : SubcommandWithHelp() {
    ...
    class CreateCommand : SubcommandWithHelp(), Callable<Int> {
        ...
        * So the ArgGroup needs and order with an even number, e.g. 6, and any group that should be placed under this group gets the order one higher, e.g. 7. That way all options appear together under the same heading.
        ...
        class MainCommand : SubcommandWithHelp(), Callable<Int> {
            
            @Option(
                names = ["--isTest"],
                description = ["If enabled, the script runs in test mode."],
                required = false
            )
            var isTest = false
            ...
            override fun call(): Int {
                ...
                checkVersionInfo()
                if (!performReleaseVerification(originalArgs)) return -1
                manageChangeProcess()
                if (!handleWebapproval()) return -1
                if (!handleOslcReports()) return -1
            ...
            private fun performReleaseVerification(originalArgs: List<String>): Boolean {
                val reportUrl =
                    if (webApprovalSection.performWebapproval or oslcSection.performOslc) {
                        ...
                        runCatching {
                            ...
                            if (webApprovalSection.performWebapproval) {
                                processWebapprovalReports(nameToAllReports)
                            }
        
                            if (oslcSection.performOslc) {
                                processOslcReports(nameToBuildReports)
                            }
            ...
            private fun processWebapprovalReports(nameToAllReports: Map<String, List<Report>>) {
            ...
            private fun processOslcReports(nameToBuildReports: Map<String, List<Report>>) {
                logger.info { "Flag --oslc found." }
            ...
            private fun handleWebapproval(): Boolean {
                val webapprovalUrl = if (webApprovalSection.performWebapproval) {
                    logger.info { "Flag --webapproval found. Adding Sharepoint entry." }
                    runCatching {
                        checkNotNull(performReleaseVerification(spec.commandLine().parseResult.originalArgs()))
                        val webapproval = webapprovalMixin.addEntry(checkNotNull(performReleaseVerification(spec.commandLine().parseResult.originalArgs())), artifactoryMixinFull.artifactoryReports)
            ...
            private fun handleOslcReports(): Boolean {
            ...
            private fun writeOutputUrls() {
                changeManagementSection.outputUrlsFile?.let { outputUrlFile ->
                    ...
                    val urlInfo = ChangeUrlFile(
                        ...
                        cdlibChangeImmutableRepoUrl = performReleaseVerification(spec.commandLine().parseResult.originalArgs()),
                        cdlibChangeWebapprovalEntryUrl = handleWebapproval(),
                        cdlibChangeOslcEntryUrls = handleOslcReports(),
