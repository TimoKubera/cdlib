...
class ChangeCommand : SubcommandWithHelp() {
    ...
    class CreateCommand : SubcommandWithHelp(), Callable<Int> {
        ...
        * So the ArgGroup needs and order with an even number, e.g. 6, and any group that should be placed under this group gets the order one higher, e.g. 7. That way all options appear together under the same heading.
        ...
        private fun logVersionInformation(versionInfo: VersionInfo) { logger.debug { "CDLib version: $versionInfo" } } 
        
        private fun verifyReports(): String? { 
            val reportUrl = 
                    ...
                    if (name !in oslcReportAppNames) { 
                        ...
                        oslcReportsForApp.forEach { report -> 
                            when (report.test.tool.name) { 
                                ...
                                else -> logger.warn { "Found OSLC-Report from unknown tool for $name" } 
                            } 
                        } 
                    } 
        ...
        private fun verifyWebApprovalReports() { 
            if (webApprovalSection.performWebapproval) { 
                ...
                val nameToAllReports = artifactoryMixinFull.artifactoryReports.groupBy { it.name } 
        ...
        private fun manageChangeProcess() { 
            runCatching { 
        ...
        private fun processOslcEntries(reportUrl: String?) : List<String> { 
        ...
        override fun call(): Int { 
            ...
            val reportUrl = verifyReports() 
            manageChangeProcess() 
        
            val webapprovalUrl = handleWebApproval(reportUrl) 
            val oslcURls = processOslcEntries(reportUrl) 
